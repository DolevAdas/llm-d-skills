...
- **The single-purpose skills own the general, guide-driven flows.** `deploy-llm-d` (guide discovery from a local `LLMD_PATH` clone) and `run-llm-d-benchmark` (drives the `llmdbenchmark` CLI with guide or custom workload profiles) are the tools when you're operating on a hand-written or pre-existing setup rather than an autoconfig-generated bundle. Phases 6–7 link out to them for those general cases.
